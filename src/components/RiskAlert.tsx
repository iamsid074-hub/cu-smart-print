...
export default function RiskAlert({ isOpen, onClose, onConfirm, level, reason, isBlocked }: RiskAlertProps) {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center px-4" style={{ background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(8px)' }}>
            <motion.div
                initial={{ opacity: 0, scale: 0.95, y: 20 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                className="bg-white w-full max-w-sm rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100"
            >
                <div className={`${config.bg} p-6 flex flex-col items-center text-center`}>
                    <div className={`w-16 h-16 rounded-2xl ${config.bg} border-2 ${config.border} flex items-center justify-center mb-4 shrink-0 shadow-sm`}>
                        <Icon className={`w-8 h-8 ${config.color}`} />
                    </div>
                    <h2 className={`text-xl font-black ${config.color} leading-tight`}>{config.title}</h2>
