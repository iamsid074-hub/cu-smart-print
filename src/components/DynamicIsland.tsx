...
export default function DynamicIsland({
  ...
  const getPageContext = () => {
    ...
    if (p === "/food")
      return {
        id: "food",
        title: "ðŸ” CU Food Menu",
        subtitle: "Order from campus restaurants",
        icon: Utensils,
        actions: [
          { label: "Browse Menu", link: "/food" },
    ...
    if (p === "/sell" || p === "/list")
      return {
        ...
        icon: Package,
        actions: [
          { label: "Quick Sell", link: "/list" },
          { label: "My Listings", link: "/profile" },
        ],
      };
    if (p === "/tracking")
      return {
        ...
        subtitle: "See all your orders",
        icon: MapPin,
        actions: [
          { label: "Active Orders", link: "/tracking" },
          { label: "Past Orders", link: "/profile" },
        ],
      };
    if (p === "/browse")
      return {
        ...
        icon: Search,
        actions: [
          { label: "Filters", link: "/browse" },
          { label: "Sort", link: "/browse" },
        ],
      };
    if (p === "/cart")
      return {
        ...
        actions: [
          ...
          { label: "Continue Shopping", link: "/browse" },
        ],
      };
    return null;
  };
    ...
    switch (status) {
      case "seller_accepted":
        return {
          ...
          desc: `Seller has accepted ${title}`,
        };
      case "confirmed":
        return {
      ...
      case "delivering":
        return {
          ...
          desc: `Your ${title} is arriving right now!`,
        };
      default:
        return null;
    }
  ...
  useEffect(() => {
    ...
    const fetchActive = async () => {
      const { data } = await supabase
        .from("orders")
        .select(
          "id, status, delivery_location, delivery_room, total_price, products(title)"
        )
        .eq("buyer_id", user.id)
        .in("status", [
          "pending",
      ...
      if (data) {
        ...
        if (
          ...
          prevOrderRef.current.id === currentOrder.id &&
          prevOrderRef.current.status !== currentOrder.status
        ) {
          const msg = getStatusMessage(currentOrder.status, currentOrder.title);
          if (msg)
            toast.success(msg.title, { description: msg.desc, duration: 5000 });
        }
    ...
    const adminOrderChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          const isFood =
            (newOrder.delivery_location?.includes("[Custom Food:") ||
              newOrder.delivery_room?.includes("[CUSTOM FOOD ORDER]")) &&
            !newOrder.product_id;
  ...
  const [activeNotif, setActiveNotif] = useState<IslandNotification | null>(
    null
  );
  const dismissTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const rapidAddRef = useRef<{
    count: number;
    totalPrice: number;
    timer: ReturnType<typeof setTimeout> | null;
  }>({ count: 0, totalPrice: 0, timer: null });
  const lastProcessedActionRef = useRef<number>(0);

  const pushNotification = useCallback(
    (notif: Omit<IslandNotification, "id">) => {
      const newNotif: IslandNotification = {
        ...notif,
  ...
  useEffect(() => {
    if (!lastAction || lastAction.timestamp <= lastProcessedActionRef.current)
      return;
  ...
  useEffect(() => {
    if (cartCount > 0) {
      ...
      setNotifications((prev) =>
        prev.filter((n) => !(n.priority === 2 && n.icon === "cart"))
      );
    }
  ...
  const handleCollapsedClick = () => {
    if (activeNotif) {
      ...
      if (
        activeNotif.type === "cart-add" ||
        activeNotif.type === "cart-remove"
      ) {
        open("cart");
        return;
      }
    }
    if (activeNotif?.type === "delivery") {
      open("delivery");
      return;
    }
    if (pageContext && !activeNotif) {
      open("context");
      return;
    }
    if (cartCount > 0 && !activeNotif) {
      open("cart");
      return;
    }
    open("search");
  ...
  const isDropdownOpen =
    view === "cart" || view === "delivery" || view === "context";
  ...
  const isPillWide =
    isExpanded || !!showingNotif || (showingIdle && !!pageContext);
  ...
  const getAnimation = () => {
    ...
    if (activeNotif?.color === "#FF6B6B")
      return "diCartPulse 1.5s ease-in-out 1";
    if (activeNotif?.color === "#30D158")
      return "diDeliveryBreathe 3s ease-in-out infinite";
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      flexShrink: 0,
                      boxShadow: `0 0 6px ${activeNotif!.color}`,
                    }}
                  />

                  {/* Icon */}
                  {activeNotif!.icon === "cart" && (
                    <ShoppingCart
                      style={{
                        width: 14,
                        height: 14,
                ...
                {view === "cart" && (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <ShoppingCart
                          style={{ width: 14, height: 14, color: "#FF6B6B" }}
                        />
                        <span
                          style={{
                            fontSize: 14,
                ...
                {view === "delivery" && activeOrder && (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <Truck
                          style={{ width: 14, height: 14, color: "#30D158" }}
                        />
                        <span
                          style={{
                            fontSize: 14,
                        ...
                        >
                          {activeOrder.status === "delivering"
                            ? "On the way!"
                            : statusLabel[activeOrder.status] || "Processing"}
                        </span>
                      </div>
                      <button
                        onClick={(e) => {
                    ...
                    >
                      {["pending", "confirmed", "picked", "delivering"].map(
                        (s, i) => {
                          const orderIdx = [
                            "pending",
                            "seller_accepted",
                    ...
                    >
                      ...
                      {activeOrder.delivery_location
                        ?.replace(/\[.*?\]/g, "")
                        .trim()}
                      {activeOrder.delivery_room &&
                      !activeOrder.delivery_room.includes("[")
                        ? `, Room ${activeOrder.delivery_room}`
                        : ""}
                    </div>
                    <Link
                ...
                {view === "context" && pageContext && (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <pageContext.icon
                          style={{ width: 14, height: 14, color: "#4DB8AC" }}
                        />
                      ...
                      {pageContext.actions.map((act, i) => (
                        <Link
                          ...
                          style={{
                            ...
                            borderRadius: 12,
                            background:
                              i === 0 ? "#4DB8AC" : "rgba(255,255,255,0.08)",
                            color: "#fff",
                            fontSize: 13,
                            fontWeight: 600,
                            textAlign: "center",
                            ...
                            boxShadow:
                              i === 0
                                ? "0 2px 10px rgba(77,184,172,0.3)"
                                : "none",
                          }}
                        >
                          {act.label}
