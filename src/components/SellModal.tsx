...
import {
  ...
  Image as ImageIcon, Package, Loader2, Phone, AlertCircle, X
} from "lucide-react";
...
export default function SellModal({ isOpen, onClose }: SellModalProps) {
  ...
  const handleNext = async () => {
    if (step === 4) {
      if (!user) { toast.error("You must be logged in to publish an item."); return; }
      ...
      if (!formData.sellerHostel.trim()) newErrors.sellerHostel = "Hostel / Block is required";
      if (!formData.sellerRoom.trim()) newErrors.sellerRoom = "Room number is required";
  ...
  function handleImageSelect(file: File) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      setImageError('Only JPG, PNG, WEBP, or GIF images are allowed.'); return;
    }
    if (file.size > MAX_SIZE_MB * 1024 * 1024) {
      setImageError(`Image must be under ${MAX_SIZE_MB}MB. Yours is ${(file.size / (1024 * 1024)).toFixed(1)}MB.`); return;
    }
    setImageFile(file);
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            transition={{ type: "spring", stiffness: 340, damping: 32, mass: 1.1 }}
            style={{
              position: "fixed",
          ...
          >
            ...
            <div style={{ padding: "10px 20px 0", flexShrink: 0 }}>
                  ...
                  <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div
                      ...
                      style={{
                        ...
                        background: step > s.id ? "#231942" : step === s.id ? "#231942" : "#f1f5f9",
                        color: step >= s.id ? "#fff" : "#94a3b8",
                        transform: step === s.id ? "scale(1.12)" : "scale(1)",
                        transition: "all 0.2s",
                        boxShadow: step === s.id ? "0 4px 12px rgba(35,25,66,0.3)" : "none",
                      }}
                    >
                      {step > s.id ? <CheckCircle style={{ width: 13, height: 13 }} /> : <s.icon style={{ width: 13, height: 13 }} />}
                    </div>
                    {i < steps.length - 1 && (
                      <div style={{ width: 24, height: 2, borderRadius: 1, overflow: "hidden", background: "#f1f5f9" }}>
                        <motion.div
                          style={{ height: "100%", background: "#231942", borderRadius: 1 }}
                          initial={{ width: "0%" }}
              ...
              <div style={{ height: 3, background: "#f1f5f9", borderRadius: 999, overflow: "hidden" }}>
                <motion.div
                  style={{ height: "100%", background: "linear-gradient(90deg, #231942, #5e4b8b)", borderRadius: 999 }}
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 20px 0" }}>
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                  ...
                  >
                    <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: 0 }}>Item Details</h2>

                    <div>
                      <label style={{ fontSize: 13, fontWeight: 700, color: "#475569", display: "block", marginBottom: 6 }}>Title *</label>
                      <input
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
                        {categories.map(cat => (
                          <button
                            ...
                            style={{
                              padding: "8px 4px", borderRadius: 12, fontSize: 12, fontWeight: 700,
                              border: formData.category === cat ? "1.5px solid #231942" : "1.5px solid #e2e8f0",
                              background: formData.category === cat ? "#231942" : "#fff",
                    ...
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 700, color: "#475569", display: "block", marginBottom: 6 }}>Condition *</label>
                        ...
                        {conditions.map(cond => (
                          <button
                            ...
                            style={{
                              ...
                              border: formData.condition === cond ? "1.5px solid #10b981" : "1.5px solid #e2e8f0",
                              background: formData.condition === cond ? "#10b981" : "#fff",
                              color: formData.condition === cond ? "#fff" : "#64748b",
                              cursor: "pointer", transition: "all 0.18s",
                            }}
                          >{cond}</button>
                        ))}
                    ...
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 700, color: "#475569", display: "block", marginBottom: 6 }}>Description</label>
                      <textarea
                        rows={3}
                ...
                {step === 2 && (
                  <motion.div
                    ...
                    transition={{ duration: 0.25 }}
                  >
                    <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: "0 0 6px" }}>Add a Photo</h2>
                    <p style={{ fontSize: 13, color: "#94a3b8", margin: "0 0 16px" }}>
                      Pick the right photo for <strong style={{ color: "#231942" }}>{formData.title || "your item"}</strong>
                    </p>

                    {imagePreview && (
                      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                        style={{ marginBottom: 16, position: "relative", borderRadius: 18, overflow: "hidden", height: 200, border: "1.5px solid #e2e8f0" }}>
                    ...
                    {imageError && (
                      <div style={{ display: "flex", gap: 8, background: "#fff1f2", borderRadius: 12, padding: 12, marginBottom: 12, border: "1px solid #fecdd3" }}>
                        <AlertCircle style={{ width: 16, height: 16, color: "#ef4444", flexShrink: 0 }} />
                        <span style={{ fontSize: 13, color: "#dc2626", fontWeight: 500 }}>{imageError}</span>
                      </div>
                    )}
                    ...
                    {!imagePreview && (
                      <div
                        ...
                        style={{
                          border: `2px dashed ${dragOver ? "#231942" : "#e2e8f0"}`,
                          borderRadius: 18, padding: "40px 16px", textAlign: "center",
                          cursor: "pointer", background: dragOver ? "#f5f3ff" : "#f8fafc",
                          transition: "all 0.2s",
                        }}
                      >
                ...
                {step === 3 && (
                  <motion.div
                    ...
                    style={{ display: "flex", flexDirection: "column", gap: 16 }}
                  >
                    <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: 0 }}>Set Your Price</h2>
                        ...
                        <div style={{ position: "relative" }}>
                          ...
                          <input
                            ...
                            style={{
                              ...
                              padding: "13px 14px 13px 30px", fontSize: 15, fontWeight: 700,
                              color: "#0f172a", border: "1.5px solid #e2e8f0", outline: "none",
                    ...
                    {formData.price && formData.originalPrice && Number(formData.originalPrice) > Number(formData.price) && (
                      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                        style={{ background: "#f0fdf4", borderRadius: 14, padding: 14, border: "1.5px solid #bbf7d0" }}>
                        <p style={{ fontSize: 12, fontWeight: 700, color: "#166534", margin: "0 0 4px" }}>Discount preview:</p>
                        <p style={{ fontSize: 26, fontWeight: 900, color: "#16a34a", margin: 0 }}>
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                      <div
                        ...
                        style={{
                          width: 44, height: 24, borderRadius: 999, transition: "all 0.25s", flexShrink: 0,
                          background: formData.negotiable ? "#231942" : "#e2e8f0", position: "relative",
                        }}
                      >
                        <div style={{
                      ...
                      <span style={{ fontSize: 14, fontWeight: 600, color: "#374151" }}>Price is negotiable</span>
                    </label>
                  </motion.div>
                )}
                ...
                {step === 4 && (
                  ...
                  >
                    <h2 style={{ fontSize: 20, fontWeight: 800, color: "#0f172a", margin: 0 }}>Your Details & Pickup</h2>

                    {/* Phone */}
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 700, color: "#475569", display: "block", marginBottom: 6 }}>Your Phone Number *</label>
                      <div style={{ position: "relative" }}>
                        <Phone style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", width: 16, height: 16, color: "#94a3b8" }} />
                        <input
                          value={formData.sellerPhone} type="tel"
                          onChange={e => { setFormData({ ...formData, sellerPhone: e.target.value }); setErrors(p => ({ ...p, sellerPhone: "" })); }}
                          placeholder="9876543210"
                          style={{
                            width: "100%", background: "#f8fafc", borderRadius: 14,
                            padding: "13px 14px 13px 40px", fontSize: 14, fontWeight: 600,
                            color: "#0f172a", border: `1.5px solid ${errors.sellerPhone ? "#ef4444" : "#e2e8f0"}`,
                            outline: "none", fontFamily: "inherit", boxSizing: "border-box",
                          }}
                        />
                      </div>
                      {errors.sellerPhone && <p style={{ fontSize: 12, color: "#ef4444", margin: "5px 0 0", fontWeight: 600 }}>{errors.sellerPhone}</p>}
                    </div>

                    {/* Hostel */}
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 700, color: "#475569", display: "block", marginBottom: 6 }}>Your Hostel / Block *</label>
                      <input
                        value={formData.sellerHostel}
                        onChange={e => { setFormData({ ...formData, sellerHostel: e.target.value }); setErrors(p => ({ ...p, sellerHostel: "" })); }}
                        placeholder="e.g. Zakir Hussain Block A"
                        style={{
                          width: "100%", background: "#f8fafc", borderRadius: 14, padding: "13px 16px",
                          fontSize: 14, fontWeight: 600, color: "#0f172a",
                          border: `1.5px solid ${errors.sellerHostel ? "#ef4444" : "#e2e8f0"}`,
                          outline: "none", fontFamily: "inherit", boxSizing: "border-box",
                        }}
                      />
                      {errors.sellerHostel && <p style={{ fontSize: 12, color: "#ef4444", margin: "5px 0 0", fontWeight: 600 }}>{errors.sellerHostel}</p>}
                    </div>

                    {/* Room */}
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 700, color: "#475569", display: "block", marginBottom: 6 }}>Your Room Number *</label>
                      <input
                        value={formData.sellerRoom}
                        onChange={e => { setFormData({ ...formData, sellerRoom: e.target.value }); setErrors(p => ({ ...p, sellerRoom: "" })); }}
                        placeholder="e.g. 402"
                        style={{
                          width: "100%", background: "#f8fafc", borderRadius: 14, padding: "13px 16px",
                          fontSize: 14, fontWeight: 600, color: "#0f172a",
                          border: `1.5px solid ${errors.sellerRoom ? "#ef4444" : "#e2e8f0"}`,
                          outline: "none", fontFamily: "inherit", boxSizing: "border-box",
                        }}
                      />
                      {errors.sellerRoom && <p style={{ fontSize: 12, color: "#ef4444", margin: "5px 0 0", fontWeight: 600 }}>{errors.sellerRoom}</p>}
                    </div>

                    {/* Meetup */}
                    <div>
                      <label style={{ fontSize: 13, fontWeight: 700, color: "#475569", display: "block", marginBottom: 8 }}>Preferred Meetup Spot</label>
                        ...
                        {["Main Gate", "Library", "Canteen", "C-Block Lobby", "Sports Complex"].map(spot => (
                          <button
                            ...
                            style={{
                              ...
                              border: formData.meetup === spot ? "1.5px solid #231942" : "1.5px solid #e2e8f0",
                              background: formData.meetup === spot ? "#231942" : "#fff",
                    ...
                    {(!formData.sellerPhone || !formData.sellerHostel || !formData.sellerRoom) && (
                      <div style={{ background: "#fffbeb", borderRadius: 12, padding: 12, border: "1.5px solid #fde68a" }}>
                        <p style={{ fontSize: 13, color: "#92400e", fontWeight: 700, margin: 0, display: "flex", alignItems: "center", gap: 6 }}>
                          <AlertCircle style={{ width: 15, height: 15, flexShrink: 0 }} /> All seller details are required to publish
                        </p>
                      </div>
                    )}
                  </motion.div>
                ...
                {step === 5 && (
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      padding: 16, display: "inline-flex", alignItems: "center", gap: 14,
                      marginBottom: 24, textAlign: "left",
                    }}>
                      <div style={{
                        width: 52, height: 52, background: "#dcfce7", borderRadius: 14,
                        display: "flex", alignItems: "center", justifyContent: "center",
                        border: "1.5px solid #bbf7d0", flexShrink: 0,
                      }}>
                        <Package style={{ width: 26, height: 26, color: "#16a34a" }} />
                      </div>
                      <div>
                    ...
                    <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
                      ...
                      <button
                        ...
                        style={{
                          padding: "12px 24px", borderRadius: 24, fontSize: 14, fontWeight: 700,
                          border: "none", background: "#231942", color: "#fff", cursor: "pointer",
            ...
            {step < 5 && (
              <div style={{
                ...
                <button
                  ...
                  disabled={step === 1}
                  style={{
                    padding: "12px 22px", borderRadius: 24, fontSize: 14, fontWeight: 700,
                ...
                <button
                  ...
                  style={{
                    padding: "12px 28px", borderRadius: 24, fontSize: 14, fontWeight: 800,
                    border: "none", background: "linear-gradient(135deg, #231942, #5e4b8b)",
                    color: "#fff", cursor: "pointer",
                    boxShadow: "0 4px 16px rgba(35,25,66,0.3)",
                    display: "flex", alignItems: "center", gap: 6,
                    opacity: (loading || (step === 4 && (!formData.sellerPhone.trim() || !formData.sellerHostel.trim() || !formData.sellerRoom.trim()))) ? 0.5 : 1,
                  }}
                >
                  {loading
