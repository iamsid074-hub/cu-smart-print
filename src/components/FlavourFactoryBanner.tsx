...
export default function FlavourFactoryBanner() {
  return (
    <div
      className="w-full h-full relative flex flex-col items-center justify-end overflow-hidden border-2 border-black rounded-[1.6rem] sm:rounded-[2.6rem] box-border"
      style={{
        background:
          "linear-gradient(160deg, #1a0a00 0%, #2d1200 30%, #3d1a00 60%, #1a0800 100%)",
      }}
    >
      {/* Warm ambient glows */}
      <div
        className="absolute top-0 left-1/4 w-72 h-72 rounded-full pointer-events-none opacity-40"
        style={{
          background:
            "radial-gradient(circle, rgba(255,120,30,0.5) 0%, transparent 70%)",
        }}
      />
      <div
        className="absolute bottom-0 right-1/4 w-60 h-60 rounded-full pointer-events-none opacity-30"
        style={{
          background:
            "radial-gradient(circle, rgba(255,60,30,0.4) 0%, transparent 70%)",
        }}
      />
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[200px] pointer-events-none opacity-15"
        style={{
          background:
            "radial-gradient(ellipse, rgba(255,180,50,0.6) 0%, transparent 70%)",
        }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center w-full h-full justify-between py-4 sm:py-8 px-4">
        ...
        >
          ...
          <span className="text-[9px] sm:text-[11px] font-black text-orange-300 uppercase tracking-[0.2em]">
            Limited Time Offer
          </span>
        </motion.div>
        ...
        <div className="text-center flex flex-col items-center gap-2 sm:gap-3">
          <motion.h2
            ...
            style={{
              backgroundImage:
                "linear-gradient(135deg, #FFD700, #FF8C00, #FFB347)",
            }}
          >
            Pasta @ {"₹"}99 Only
          </motion.h2>
