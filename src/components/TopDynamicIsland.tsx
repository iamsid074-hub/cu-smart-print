...
type IslandState = "default" | "browsing" | "explore" | "cart" | "profile" | "added" | "updated" | "grocery" | "sell" | "offer";
...
export default function TopDynamicIsland({ onSell }: TopDynamicIslandProps) {
  ...
  useEffect(() => {
    if (location.pathname.startsWith("/food")) {
      ...
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
    } else if (location.pathname === "/pasta-offer") {
      setIslandState("offer");
      if (timeoutRef.current) clearTimeout(timeoutRef.current);
  ...
  switch (islandState) {
    ...
    case "offer":
      ...
      content = (
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold tracking-wide text-white/90">Offer</span>
        </div>
      );
      break;
  ...
  return (
    <>
      {/* Apple-style premium liquid glass blur background for the top header */}
      <div 
        className="fixed top-0 left-0 right-0 h-20 sm:h-24 z-[9998] pointer-events-none border-b border-white/[0.08]"
        style={{ 
