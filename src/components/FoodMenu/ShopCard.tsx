...
export const ShopCard = memo(({
  ...
  return (
    <>
      {/* ── Card Row: Main Image + See Menu ── */}
      <div className="flex gap-2.5 w-full">
        {/* Main Image Card */}
        <div
          className="flex-1 rounded-[1.4rem] overflow-hidden bg-[#1c1c1e] border border-white/5 relative"
        ...
        {/* Small "See Menu" side card */}
        <button
          className="w-[72px] sm:w-[80px] rounded-[1.4rem] overflow-hidden border border-white/5 flex flex-col items-center justify-center gap-2 transition-all active:scale-95 shrink-0"
          style={{
            background: isExpanded ? accent.grad : "#1c1c1e",
            boxShadow: isExpanded
              ? `0 6px 20px ${accent.dot}40`
        ...
        >
          <div
            ...
            style={{
              background: isExpanded ? "rgba(255,255,255,0.2)" : accent.light,
            }}
          >
            {isExpanded
              ? <ChevronUp className="w-4 h-4 text-white" />
              : <ChevronDown className="w-4 h-4" style={{ color: accent.text }} />}
          </div>
          <span
            className="text-[10px] font-bold leading-tight text-center px-1"
            style={{ color: isExpanded ? "#fff" : accent.text }}
          >
            {isExpanded ? "Close" : "See Menu"}
          </span>
      ...
      {/* ── Expanded Menu (slides below the card row) ── */}
