        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h3 className="text-[17px] sm:text-[19px] font-bold text-[#1D1D1F] tracking-tight truncate">
                {shop.name}
              </h3>
              {shop.veg && (
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#34C759]/10 text-[#34C759] border border-[#34C759]/20 uppercase tracking-tight">
                  VEG
                </span>
              )}
            </div>
          ...
          {isExpanded && (
            ...
            >
              <div className="px-5 pb-6 sm:px-6 sm:pb-8 space-y-4">
                <div className="flex gap-2.5 overflow-x-auto scrollbar-hide py-4 -mx-1 px-1">
                  {shop.categories.map((mc) => (
                    <button
                      ...
                      onClick={() =>
                        onToggleCategory(
                          expandedMenuCat === mc.category ? null : mc.category
                        )
                      }
                ...
                {shop.categories
                  .filter(
                    (mc) => !expandedMenuCat || mc.category === expandedMenuCat
                  )
                  .map((mc) => (
                    <div key={mc.category} className="mb-6">
                      <div className="flex items-center gap-3 mb-3 px-1">
                        <div className="h-[2px] w-4 bg-[#8E8E93]/30 rounded-full" />
                        <p className="text-[11px] font-bold text-[#8E8E93] uppercase tracking-[0.1em]">
                          {mc.category}
                        </p>
                      </div>
                        ...
                        {mc.items.map((item, idx) => (
                          ...
                          >
                            <div className="flex-1 min-w-0 pr-3">
                              <span className="text-[15px] font-bold text-[#1D1D1F] block truncate tracking-tight">
                                {item.name}
                              </span>
                              <span className="text-[13px] font-bold text-[#007AFF] mt-0.5 block">
                                {"₹"}
                                {item.price}
                              </span>
                            </div>
