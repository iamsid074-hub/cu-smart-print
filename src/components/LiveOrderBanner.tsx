...
> = {
  ...
  delivering: {
    label: "Out for Delivery ðŸ›µ",
    color: "text-brand",
    bg: "bg-brand-50 border-brand-muted",
    pulse: "bg-brand",
    icon: Truck,
  },
  completed: {
  ...
  useEffect(() => {
    ...
    const fetchActive = async () => {
      const { data } = await supabase
        .from("orders")
        .select(
          "id, status, total_price, delivery_location, delivery_room, products(title)"
        )
        .eq("buyer_id", user.id)
        .in("status", [
          "pending",
    ...
    <AnimatePresence>
      ...
      >
        <Link to={`/tracking?order=${order.id}`}>
          <div
            className={`rounded-2xl border p-3 flex items-center gap-3 shadow-md ${
              config.bg
            } ${
              justChanged ? "ring-2 ring-offset-1 ring-brand" : ""
            } transition-all`}
          >
            {/* Animated status dot */}
