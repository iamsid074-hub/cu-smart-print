...
function FloatingOrb({ color, size, top, left, delay }: { color: string; size: number; top: string; left: string; delay: number }) {
  return (
    <motion.div
      ...
      style={{ width: size, height: size, top, left, background: `radial-gradient(circle at 40% 40%, ${color}, transparent 70%)`, filter: "blur(60px)" }}
...
export default function Login() {
  ...
  const glassInput = (focused: boolean): React.CSSProperties => ({
    background: focused ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.65)",
  ...
  return (
    ...
    >
      {/* Ambient orbs */}
      <FloatingOrb color="rgba(124,58,237,0.25)" size={500} top="-10%" left="-10%" delay={0} />
      <FloatingOrb color="rgba(255,107,53,0.2)" size={400} top="60%" left="55%" delay={2} />
      <FloatingOrb color="rgba(5,150,105,0.15)" size={350} top="80%" left="-5%" delay={4} />
      ...
      <AnimatePresence>
        {showIntro && (
          <motion.div
            ...
            style={{ background: "linear-gradient(135deg, #f8faff 0%, #eef2ff 50%, #fdf4ff 100%)" }}
          >
            <FloatingOrb color="rgba(124,58,237,0.25)" size={400} top="0%" left="0%" delay={0} />
            <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", damping: 18 }} className="relative">
              <div className="w-24 h-24 rounded-[2rem] overflow-hidden shadow-2xl" style={{ boxShadow: "0 20px 60px rgba(124,58,237,0.3)" }}>
                <img src="/logo.webp" alt="CU Bazzar" className="w-full h-full object-cover" />
              </div>
              <motion.div className="absolute inset-0 rounded-[2rem]" animate={{ boxShadow: ["0 0 0 0px rgba(124,58,237,0.3)", "0 0 0 24px rgba(124,58,237,0)"] }} transition={{ duration: 1.5, repeat: Infinity }} />
            </motion.div>
            <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="mt-6 text-2xl font-black" style={{ ...fontDisplay, color: "#0F0A1E" }}>CU BAZZAR</motion.p>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }} className="text-[10px] font-bold uppercase tracking-[0.3em] mt-1" style={{ color: "#7C3AED" }}>EOS v3</motion.p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        <div
          ...
          style={{
            ...
            boxShadow: "0 30px 80px rgba(124,58,237,0.12), 0 0 0 1px rgba(255,255,255,0.6)",
          }}
        >
          {/* Logo */}
          ...
          {!forgotPassword && (
            <motion.button
              ...
              whileHover={{ y: -1, boxShadow: "0 12px 30px rgba(0,0,0,0.1)" }}
              whileTap={{ scale: 0.97 }}
              className="w-full flex items-center justify-center gap-3 h-[52px] rounded-2xl mb-5 font-semibold text-sm transition-all"
              style={{ background: "rgba(255,255,255,0.9)", border: "1.5px solid rgba(255,255,255,0.9)", boxShadow: "0 4px 16px rgba(0,0,0,0.06)", color: "#0F0A1E" }}
            >
          ...
          <AnimatePresence mode="wait">
            {forgotPassword ? (
              <motion.div key="forgot-form" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                {resetSent ? (
                  ...
                  <form onSubmit={handleForgotPassword} className="space-y-4">
                    ...
                    <motion.button type="submit" disabled={loading} whileTap={{ scale: 0.97 }} whileHover={{ y: -1 }} className="w-full h-[52px] rounded-2xl font-bold text-[15px] text-white flex items-center justify-center gap-2 disabled:opacity-60" style={{ background: "#0F0A1E", boxShadow: "0 4px 20px rgba(15,10,30,0.18), inset 0 1px 0 rgba(255,255,255,0.08)" }}>
              ...
              <motion.form key="main-form" onSubmit={handleSubmit} className="space-y-4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                {/* Email */}
                ...
                {!isLogin && (
                  <div className="flex items-start gap-3 px-1">
                    <button type="button" onClick={() => setAcceptedTerms(!acceptedTerms)} className="w-5 h-5 rounded-md flex-shrink-0 mt-0.5 flex items-center justify-center transition-all" style={{ background: acceptedTerms ? "#7C3AED" : "rgba(255,255,255,0.8)", border: `1.5px solid ${acceptedTerms ? "#7C3AED" : "rgba(15,10,30,0.15)"}` }}>
                ...
                <AnimatePresence>
                  {formError && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-center gap-2.5 p-3 rounded-xl overflow-hidden" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)" }}>
                ...
                {/* Submit */}
                <motion.button type="submit" disabled={loading} whileHover={{ y: -1 }} whileTap={{ scale: 0.97 }} className="w-full h-[52px] rounded-2xl font-bold text-[15px] text-white flex items-center justify-center gap-2 disabled:opacity-60 mt-2 relative overflow-hidden group" style={{ background: "#0F0A1E", boxShadow: "0 4px 24px rgba(15,10,30,0.18), inset 0 1px 0 rgba(255,255,255,0.08)" }}>
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl" style={{ background: "rgba(255,255,255,0.05)" }} />
        ...
        <div className="text-center mt-5">
          <Link to="/" className="text-xs font-semibold" style={{ color: "rgba(15,10,30,0.35)" }}>
            ← Back to Home
          </Link>
        </div>
      </motion.div>
