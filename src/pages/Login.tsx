...
export default function Login() {
    ...
    return (
        <div className="min-h-screen bg-[#08080c] flex flex-col lg:flex-row overflow-hidden">
                ...
                >
                    ...
                    >
                        <div className="flex -space-x-2.5">
                            {["from-violet-500 to-fuchsia-500", "from-cyan-500 to-blue-500", "from-amber-500 to-orange-500", "from-emerald-500 to-teal-500"].map((bg, i) => (
                                <div key={i} className={`w-8 h-8 rounded-full bg-gradient-to-br ${bg} border-2 border-[#08080c] flex items-center justify-center`}>
            ...
            {/* ─── Right Panel — Form ─── */}
            <div className="flex-1 flex items-center justify-center px-5 py-10 sm:px-8 lg:px-16 relative min-h-screen lg:min-h-0">
                {/* Mobile ambient glow */}
                <div className="lg:hidden absolute top-0 right-0 w-60 h-60 bg-violet-500/12 rounded-full blur-[80px] pointer-events-none" />
                <div className="lg:hidden absolute bottom-0 left-0 w-60 h-60 bg-cyan-500/8 rounded-full blur-[80px] pointer-events-none" />
                ...
                >
                    ...
                    <div className="lg:hidden flex items-center gap-2.5 mb-8">
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center" style={{ borderRadius: "10px" }}>
                            <Sparkles className="w-4.5 h-4.5 text-white" />
                        </div>
                        <span className="text-lg font-bold text-white">CU Bazzar</span>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className={`relative rounded-xl transition-all duration-300 ${focusedField === "email"
                                ? "ring-2 ring-violet-500/40 shadow-[0_0_16px_rgba(139,92,246,0.1)]"
                                : "ring-1 ring-white/[0.08]"
                                }`}>
                                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                    <Mail className={`h-4 w-4 transition-colors duration-200 ${focusedField === "email" ? "text-violet-400" : "text-white/20"}`} />
                                </div>
                                <input
                                    type="email"
                                    required
                                    ...
                                    className="w-full bg-white/[0.03] rounded-xl pl-10 pr-4 h-[52px] text-sm text-white focus:outline-none transition-colors placeholder:text-white/20"
                        ...
                        <div>
                            ...
                            <div className={`relative rounded-xl transition-all duration-300 ${focusedField === "password"
                                ...
                                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                    <Lock className={`h-4 w-4 transition-colors duration-200 ${focusedField === "password" ? "text-violet-400" : "text-white/20"}`} />
                                </div>
                                <input
                                    type="password"
                                    required
                            ...
                            {!isLogin && (
                                <p className="text-[11px] text-white/25 mt-1 ml-0.5">At least 6 characters</p>
                            )}
                        </div>
                    ...
                    {/* Footer */}
                    <p className="text-[11px] text-white/15 mt-6 leading-relaxed">
