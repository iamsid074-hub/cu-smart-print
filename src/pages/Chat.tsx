...
export default function Chat() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-16 flex">
      {/* Sidebar */}
      <div className={`w-full md:w-80 flex-shrink-0 flex flex-col border-r border-slate-100 bg-white ${activeContact ? "hidden md:flex" : "flex"}`}>
        {/* Header */}
        <div className="p-4 border-b border-slate-100">
          <h2 className="font-bold text-lg mb-3 text-slate-900">Messages</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          {profiles.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-6 opacity-60">
              ...
              <p className="text-sm font-medium text-slate-700">{searchQuery ? "No users found" : "No active chats"}</p>
              <p className="text-xs text-slate-400 mt-1">{searchQuery ? "Try a different search term" : "Search for a user to start chatting!"}</p>
            </div>
          ) : (
      ...
      {activeContact ? (
        <div className={`flex-1 flex flex-col ${!activeContact ? "hidden md:flex" : "flex"}`}>
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 bg-white shadow-sm">
            <button onClick={() => setActiveContact(null)} className="md:hidden p-1.5 rounded-full hover:bg-slate-100 text-slate-500 transition-colors">
            ...
            <div className="flex-1 leading-tight">
              <p className="font-semibold text-sm text-slate-900">{activeContact.full_name}</p>
              <div className="flex items-center gap-2">
                <p className="text-xs text-slate-500 font-medium">@{activeContact.username || "unknown"}</p>
                <p className="text-xs text-emerald-500 font-semibold">{activeContact.online ? "● Online" : ""}</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              <button className="p-2 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors"><Phone className="w-4 h-4" /></button>
              <button className="p-2 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors"><Video className="w-4 h-4" /></button>
              <button className="p-2 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-700 transition-colors"><MoreVertical className="w-4 h-4" /></button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
            {messages.length === 0 && (
              <div className="text-center mt-10">
                <span className="text-xs text-slate-400 bg-slate-100 px-4 py-1.5 rounded-full">Say hello to {activeContact.full_name}! 👋</span>
              </div>
            )}
          ...
          <div className="p-4 border-t border-slate-100 bg-white">
            <div className="flex items-center gap-3 bg-slate-50 rounded-full px-5 py-2.5">
