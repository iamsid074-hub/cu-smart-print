...
export default function Chat() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-16 pb-32 flex">
      {/* Sidebar */}
      <div
        className={`w-full md:w-80 flex-shrink-0 flex flex-col border-r border-slate-100 ${
            ...
            profiles.map((profile, i) => (
              <motion.button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors text-left ${
                  activeContact?.id === profile.id
                    ? "bg-brand-50 border-r-2 border-brand-accent"
                    : ""
                }`}
              >
                {/* Avatar */}
              ...
              {activeContact.avatar_url ? (
                ...
                <div
                  className={`w-10 h-10 rounded-full bg-gradient-to-br ${
                    avatarGradients[
                      Math.abs(activeContact.id.charCodeAt(0)) % 5
                    ]
                  } flex items-center justify-center text-white text-xs font-bold`}
                >
