    ...
    if (step === 4) {
      ...
      try {
          ...
          try {
            ...
            alert(
              "Image compression failed: " +
                (compressErr?.message || "Unknown error")
            );
            setLoading(false);
            return;
          }
        ...
        if (error) {
          ...
          alert(
            "Product insert failed: " + (error.message || JSON.stringify(error))
          );
          setLoading(false);
          return;
        }
            ...
            {steps.map((s, i) => (
              <div key={s.id} className="flex items-center gap-2">
                <motion.div
                  ...
                  className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 cursor-pointer ${
                    step > s.id
                      ? "bg-brand text-white shadow-[0_4px_16px_rgba(35,25,66,0.3)]"
                      : step === s.id
                      ? "bg-brand text-white shadow-[0_4px_16px_rgba(35,25,66,0.3)] scale-110"
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 1 && (
              <motion.div
                ...
                className="space-y-5"
              >
                <h2 className="font-extrabold text-2xl mb-6 text-slate-900">
                  Item Details
                </h2>
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                    {categories.map((cat) => (
                      <button
                        ...
                        className={`px-3 py-2.5 rounded-2xl text-[13px] font-bold transition-all duration-200 ${
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    {conditions.map((cond) => (
                      <button
                        ...
                        className={`px-4 py-3 rounded-2xl text-[13px] font-bold transition-all ${
            ...
            {step === 2 && (
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <h2 className="font-extrabold text-2xl mb-2 text-slate-900">
                  Add a Photo
                </h2>
                <p className="text-sm text-slate-500 mb-6">
                  Pick the correct image for{" "}
                ...
                {imagePreview && (
                  <motion.div
                    ...
                    className="mb-6 relative rounded-3xl overflow-hidden border border-slate-200 shadow-sm"
                    style={{ height: 240 }}
                  >
                    <img
                      src={imagePreview}
                ...
                {imageError && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="mb-5 flex items-start gap-2 bg-rose-50 rounded-2xl p-4 border border-rose-100"
                  >
                ...
                <div
                  ...
                  className={`border-2 border-dashed rounded-3xl p-10 text-center transition-all duration-300 cursor-pointer ${
                    dragOver
                      ? "border-brand-light bg-brand-50"
                      : imagePreview
                ...
                >
                  <Upload
                    className={`w-12 h-12 mx-auto mb-4 transition-colors ${
                      dragOver ? "text-brand-accent" : "text-slate-400"
                    }`}
                  />
                  <p className="font-bold text-slate-700 mb-1.5 text-lg">
                ...
                <p className="text-xs text-slate-500 mt-3 text-center">
                  ðŸ’¡ Check the preview above to confirm it's the correct photo
                  before continuing.
                </p>
              </motion.div>
            )}
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <div
                    ...
                    className={`w-12 h-6 rounded-full transition-all duration-300 ${
                      formData.negotiable ? "bg-gradient-fire" : "bg-secondary"
                    }`}
                  >
                    <div
                      className={`w-5 h-5 rounded-full bg-white shadow transition-all duration-300 mt-0.5 ${
                        formData.negotiable ? "ml-6.5" : "ml-0.5"
                      }`}
                      style={{
                        marginLeft: formData.negotiable
                          ? "calc(100% - 1.375rem)"
                          : "2px",
                      }}
                    />
              ...
              >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setErrors((prev) => ({ ...prev, sellerPhone: "" }));
                      }}
                      placeholder="e.g. 9876543210"
                      type="tel"
                      className={`w-full bg-slate-50 rounded-2xl pl-11 pr-5 py-4 text-base font-bold text-slate-900 placeholder:text-slate-400 outline-none border transition-all ${
                        errors.sellerPhone
                          ? "border-rose-400 focus:border-rose-500 focus:ring-4 focus:ring-rose-50"
                          : "border-slate-200 focus:border-brand-accent focus:ring-4 focus:ring-brand-50"
                      }`}
                    />
                  </div>
                  {errors.sellerPhone && (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setErrors((prev) => ({ ...prev, sellerHostel: "" }));
                    }}
                    placeholder="e.g. Zakir Hussain Block A"
                    className={`w-full bg-slate-50 rounded-2xl px-5 py-4 text-base font-bold text-slate-900 placeholder:text-slate-400 outline-none border transition-all ${
                      errors.sellerHostel
                        ? "border-rose-400 focus:border-rose-500 focus:ring-4 focus:ring-rose-50"
                        : "border-slate-200 focus:border-brand-accent focus:ring-4 focus:ring-brand-50"
                    }`}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setFormData({ ...formData, sellerRoom: e.target.value });
                      setErrors((prev) => ({ ...prev, sellerRoom: "" }));
                    }}
                    placeholder="e.g. 402"
                    className={`w-full bg-slate-50 rounded-2xl px-5 py-4 text-base font-bold text-slate-900 placeholder:text-slate-400 outline-none border transition-all ${
                      errors.sellerRoom
                ...
                {(!formData.sellerPhone ||
                  ...
                  <div className="bg-amber-50 rounded-2xl p-4 border border-amber-200">
                    <p className="text-sm font-bold text-amber-600 flex items-center gap-1.5">
                      <AlertCircle className="w-4 h-4" /> All seller details are
                      required to publish
                    </p>
                  </div>
                )}
              ...
              >
                ...
                <div>
                  <button
                    ...
                    className="px-8 py-4 bg-brand hover:bg-brand text-white font-bold rounded-full shadow-md transition-all"
                  >
                    Keep Selling
                  </button>
                </div>
