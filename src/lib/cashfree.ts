...
export async function createCashfreeOrder(params: {
    ...
    order_note?: string;
}): Promise<{ payment_session_id: string; cf_order_id: string; order_id: string }> {
...
export async function verifyCashfreePayment(orderId: string): Promise<{
    ...
    const res = await fetch(`${API_BASE}/api/cashfree-verify?order_id=${encodeURIComponent(orderId)}`);
    const data = await res.json();
...
export function loadCashfreeSDK(): Promise<any> {
    ...
    sdkPromise = new Promise((resolve, reject) => {
        ...
        if ((window as any).Cashfree) {
            ...
            resolve((window as any).Cashfree({ mode: env === 'production' ? 'production' : 'sandbox' }));
            return;
        }
