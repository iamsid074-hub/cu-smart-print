...
const BLACKLISTED_KEYWORDS = [
    "abuse", "fraud", "spam", "test", "fake", "admin", "system",
    // Adding common local variations (Hindi/Slang) based on typical bot/abusive patterns
    "teri", "chut", "madar", "bhen", "bc", "mc", "lola", "gand", "pussy", "dick", "sex",
    "bazzar", "cu", "smart", "print"
];
...
export const evaluateOrderRisk = (
    ...
    if (containsAbuse(userName)) {
        return {
            ...
            reason: "Account or username contains restricted keywords. Please update your profile.",
            level: "high"
        };
    }
    ...
    if (totalOrders === 0 && totalPrice > 500) {
        return {
            ...
            reason: "High-value first order detected. Please confirm you want to proceed with this order.",
            level: "medium"
        };
    }
    ...
    if (totalPrice > 2500) {
        return {
            isBlocked: false,
            requiresConfirmation: true,
            reason: "This is a large order. A quick confirmation helps us prepare your items faster.",
            level: "low"
        };
    }
