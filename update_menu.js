...
for (let i = 0; i < lines.length; i++) {
    ...
    if ((insideChatori1 || insideChatori2) && lines[i].includes('categories: [')) {
        newLines.push(newCategories);
        let braceLevel = 0;
        let started = false;
        let j = i;
        while (j < lines.length) {
            ...
            if (started && braceLevel === 0) {
                i = j;
                if (insideChatori1) insideChatori1 = false;
                if (insideChatori2) insideChatori2 = false;
                break;
            }
            j++;
        }
